...
void gen_circle(const Vertex2i& center, int radius, int half_segments = 32 ) {
  double angle = 0.0;
  double angle_increment = M_PI / half_segments;

  for (auto i=0; i<=2*half_segments; i++) {
     points.emplace_back(center.x + radius*cos(angle),center.y + radius*sin(angle));
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
  ...
  gen_circle(Vertex2i(WIN_WIDTH/2,WIN_HEIGHT/2),WIN_WIDTH/3, 50);
